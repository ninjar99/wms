...
public class JFilterComboBox extends JComboBox {  
    ...
    /** 
     * 创建一个JFilterComboBox， 
     * 其项取自现有的ComboBoxModel。 
     * 由于提供了ComboBoxModel， 
     * 使用此构造方法创建的组合框不创建默认组合框模型， 
     * 这可能影响插入、移除和添加方法的行为方式。 
     * @param aModel - 提供显示的项列表的 ComboBoxModel 
     */  
    ...
    /** 
     *  创建包含指定 Vector 中的元素的 JFilterComboBox。 
     * 默认情况下，选择数组中的第一项（因而也选择了该项的数据模型）。  
    ...
    /** 
     * 创建具有默认数据模型的 JFilterComboBox。 
     * 默认的数据模型为空对象列表。使用 addItem 添加项。 
     * 默认情况下，选择数据模型中的第一项。 
     */  
